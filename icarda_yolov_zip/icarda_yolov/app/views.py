...
def get_visitor_ip(request):
    ...
    server_details={}
    server_details['x_forwarded_for'] = request.META.get('HTTP_X_FORWARDED_FOR')
    server_details['server_name'] = request.META.get('SERVER_NAME')
    server_details['server_port'] = request.META.get('SERVER_PORT')
    server_details['remote_host'] = request.META.get('REMOTE_HOST')
    if server_details['x_forwarded_for'] is not None:
        server_details['x_forwarded_for'] = server_details['x_forwarded_for'].split(',')[0]
    else:
        server_details['x_forwarded_for'] = request.META.get('REMOTE_ADDR')
    return server_details

def save_ip(request):
    ip = get_visitor_ip(request)
    visitor = visitor_detials(visitor_ip_addr=ip['x_forwarded_for'],server_name=ip['server_name'],server_port=ip['server_port'],remote_host=ip['remote_host'])
    result = visitor_detials.objects.filter(visitor_ip_addr=ip['x_forwarded_for'])
    if len(result) == 1:
